...
CliRunner.prototype = {
  ...
  parseTestSettings : function(callback) {
    ...
    if (!this.isParallelMode() && envs.length < 2
      ...
      this.setupTestWorkerMode(callback);
    }
  ...
  setTestSettings : function(env) {
    ...
    if (this.argv.skiptag) {
      this.test_settings.skiptag_filter = this.argv.skiptag;
    }

    console.log("Failed Tests", this.argv.failedTests);
    if(this.argv.failedTests){
      this.test_settings.filter_tests = this.argv.failedTests.split(',');
    }
  ...
  setupParallelMode : function(envs, done) {
    console.log('Parallel mode set', envs);
  ...
  setupTestWorkerMode: function(callback) {
    ...
    this.startSelenium(function() {
      self.startTestWorkers(function(failedTests, code) {

        console.log("Results ", failedTests);

        //Writing out a failed test workers json file.

        fs.writeFile(self.output_folder + "/failedTests.json", JSON.stringify(failedTests), function(){
          self.stopSelenium();
          if (callback) {
            callback();
          }
  ...
  startTestWorkers : function(finishCallback) {
    ...
    /**
     * Checking if the file should be pushed to the queue.
     *
     * Currently verifying tags and filename.
     *
     * TODO: Verify filter by group.
     * @param baseName
     * @param file
     */
    function checkFilePush(basename, file){
      var validFile = false;
      if (self.test_settings.filter_tests){
        return self.test_settings.filter_tests.indexOf(basename) > -1;
      }

      if (self.test_settings.filename_filter){
        validFile = validFile || self.test_settings.filename_filter.indexOf(basename) > -1;
      } else if(self.test_settings.tag_filter){
        validFile = validFile || fileMatcher.tags.match(file, self.test_settings.tag_filter);
      } else { // only js files
        validFile = validFile || path.extname(basename) === '.js';
      }

      //If the tag is to be skipped, mark it as invalid
      if(self.test_settings.skiptag_filter){
        validFile = validFile && !fileMatcher.tags.match(file, self.test_settings.skiptag_filter);
      }
        ...
        list.forEach(function(file) {
          ...
          fs.stat(file, function(err, stat) {
            if (stat && stat.isDirectory()) {
              ...
              //Checking if the file should be pushed to the queue.
              if(checkFilePush(basename, file)){
    ...
    walk(paths[0], function(err, tests) {
      if (err) {return console.error('Error walking: ', err);}
      ...
      queue.drain = function() {
        if (!self.processesRunning()) {
          finishCallback(failedTests, globalExitCode);
        }
      };
    }, this.test_settings);
  },
