  ...
  this.assert = function assert(passed, receivedValue, expectedValue, message, abortOnFailure) {
    ...
    if (!passed) {
      if (client.options.screenshots.onFail) {
        ...
        var fileName = 'FAIL__' + client.api.currentTest.module + '.js__' + client.api.currentTest.name + '__' + (message ? message.replace(/[^a-z0-9]/gi, '_') : '') + '_' + theBrowser + '_' + dateStamp + '.png';
        // e.g. FAIL__my_test_file__Test_A_Thing__Timed_out_while_waiting_for_element___test__to_be_present_for_10000_milliseconds_firefox_Fri-Apr-03-2015-001327-GMT+0100.png
        var filePath = path.resolve(path.join(client.options.screenshots.path, fileName));
        client.api.saveScreenshot(filePath, function(result) {
          var currentResult = client.results.tests[client.results.tests.length -1];
          if (currentResult) {
            currentResult.screenshots = currentResult.screenshots || [];
            currentResult.screenshots.push(filePath);
          }
          if (abortOnFailure) {
            client.terminate();
          }
